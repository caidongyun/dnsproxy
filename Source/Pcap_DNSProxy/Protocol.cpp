						...
						(ResultBlacklistTableIter.PatternString.empty() || std::regex_match(DomainString, ResultBlacklistTableIter.PatternRegex)))
					{
						for (const auto &AddressRangeTableIter:ResultBlacklistTableIter.Addresses)
						{
							if ((AddressRangeTableIter.End.ss_family == AF_INET6 && 
								AddressesComparing(AF_INET6, OriginalAddr, &((PSOCKADDR_IN6)&AddressRangeTableIter.Begin)->sin6_addr) >= ADDRESS_COMPARE_EQUAL && 
								AddressesComparing(AF_INET6, OriginalAddr, &((PSOCKADDR_IN6)&AddressRangeTableIter.End)->sin6_addr) <= ADDRESS_COMPARE_EQUAL) || 
						...
						(ResultBlacklistTableIter.PatternString.empty() || std::regex_match(DomainString, ResultBlacklistTableIter.PatternRegex)))
					{
						for (const auto &AddressRangeTableIter:ResultBlacklistTableIter.Addresses)
						{
							if ((AddressRangeTableIter.End.ss_family == AF_INET && 
								AddressesComparing(AF_INET, OriginalAddr, &((PSOCKADDR_IN)&AddressRangeTableIter.Begin)->sin_addr) >= ADDRESS_COMPARE_EQUAL && 
								AddressesComparing(AF_INET, OriginalAddr, &((PSOCKADDR_IN)&AddressRangeTableIter.End)->sin_addr) <= ADDRESS_COMPARE_EQUAL) || 
		...
		//AAAA Records
			...
			//Check addresses.
				if ((Parameter.DataCheck_Blacklist && CheckSpecialAddress(AF_INET6, Buffer + DataLength, false, DomainString)) || 
					...
					ResponseType == REQUEST_PROCESS_LOCAL && !CheckAddressRouting(AF_INET6, Buffer + DataLength)))
						return EXIT_FAILURE;
		...
		//A Records
			...
			//Check addresses.
				if ((Parameter.DataCheck_Blacklist && CheckSpecialAddress(AF_INET, Buffer + DataLength, false, DomainString)) || 
					(Index < ntohs(DNS_Header->Answer) && !Parameter.IsLocalHosts && Parameter.IsLocalRouting && 
					ResponseType == REQUEST_PROCESS_LOCAL && !CheckAddressRouting(AF_INET, Buffer + DataLength)))
						return EXIT_FAILURE;
...
//Additional EDNS Label Resource Records check, DNSSEC Validation check and Local request result check
	if (
		...
		(ResponseType == REQUEST_PROCESS_LOCAL && !Parameter.IsLocalForce && !IsGotAddressResult)
	#if defined(ENABLE_LIBSODIUM)
		)
	#endif
		)
